...
<head>
    ...
    $dp_frm = (isset($_GET['from_dp'])) ? $_GET['from_dp'] : ((isset($_POST['from_dp'])) ? $_POST['from_dp'] : '---');
    ...
    if ($submit == "OK") { //================== INSERT OR UPDATE ====================
        if ($Sel_Boxid == "") { //===== INSERT ==========
            ...
            if ($yy >= 1) {
                ...
                $rsLog = mysqli_query($dbconn, "INSERT INTO u_log SET id_user= '" . $_SESSION['s_id'] . "' ,session='_add',desc_log='$descLog'")
                    or die(mysqli_error($dbconn . " Q=" . $rsLog));
            }
        } else {        //===== UPDATE ==========
            if ($box_id_old == $boxid) {  //===== ตรวจสอบเปลี่ยนข้อมูลกล่องเดิมหรือไม่ (ใช่)
                $sqlUp = "UPDATE pro_pack SET box_id='$boxid',prod_id='$prod',col_id='$colid',rm_id='$rm',roll_id='$roll',roll_n='$nroll',prod_kg='$prodkg',val='$in_dex',status='0', to_dp='$to_dp', pack_bill='$p_bill'  WHERE(pack_date='$date_pack' AND from_dp='$dp_frm' AND box_id='$Sel_Boxid')";
                ...
                $rsLog = mysqli_query($dbconn, "INSERT INTO u_log SET id_user= '" . $_SESSION['s_id'] . "' ,session='_edit',desc_log='$descLog'")
                    or die(mysqli_error($dbconn . " Q=" . $rsLog));
    ...
    if ($submit == "DEL") { //===== DELETE ==========
        $sqldel = "DELETE FROM pro_pack WHERE(pack_date='$date_pack' AND from_dp='$dp_frm' AND box_id = '$Sel_Boxid')";
        ...
        $rsLog = mysqli_query($dbconn, "INSERT INTO u_log SET id_user= '" . $_SESSION['s_id'] . "' ,session='_del',desc_log='$descLog'")
            or die(mysqli_error($dbconn . " Q=" . $rsLog));
    }
    ?>
</head>
...
<body>
    <div class="container">
        <div class="row">
            <div class="col-md-12" style='margin-bottom: 8px; font-size:18px'>
                <span class="d-block p-2 bg-warning text-white" align="center"><?php echo $_SESSION['s_depart'] ?> <i class="fas fa-cube fa-lg" aria-hidden="true"></i> บรรจุวัตถุดิบ </span>
            </div>
            <!-- ================================== Show Form ========================================= -->
            <?php
            if ($submit == "" or $show == "OK") { ?>
                <div class="col-md-12">
                    <form name="fmSearch" id="fmSearch" method="post" action="rm_pack_admin?show=OK&strSearch=Y" role='Search'>
                        <div class="form-group row">
                            ...
                            <div class="input-group mb-2 col-md-6">
                                <div class="input-group-prepend">
                                    <label class="input-group-text" for="from_dp">ผู้บรรจุ</label>
                                </div>
                                <select class="custom-select" id="from_dp" name="from_dp" onchange="submitS()" tabIndex="2">
                                    <option selected>---</option>
                                    <?php
                                    $rstTemp = mysqli_query($dbconn, 'SELECT * FROM depart');
                                    while ($arr_2 = mysqli_fetch_array($rstTemp)) {
                                    ?>
                                        <option value="<?php echo ($arr_2['id_depart']) ?>" <?php if ($dp_frm == $arr_2['id_depart']) {
                                                                                                echo ('selected');
                                                                                            } ?>>
                ...
                $cal_sql = "SELECT pack_date, pack_bill ,box_id, roll_n, prod_kg, from_dp, prod_name, col_code, rm_code, roll_type, val, pp.status, depart
                    ...
                    INNER JOIN roll_type ON pp.roll_id = roll_type.roll_id WHERE(from_dp='$dp_frm' AND pack_date='$date_pack' AND CONCAT(depart,' $',LPAD(pack_bill,2,'0'),' #',LPAD(box_id,3,'0'),' ',IF(status = 0,'+n','+y'),' ',prod_name,' ',col_code,' ',rm_code,' ',roll_type,' ',prod_type) like '%$search%')";
                ...
                <div class="col-md-12">
                    <table class="table table-striped table-responsive">
                        <thead>
                            <tr>
                                ...
                                <td align='center'><a title='พิมพ์บาร์โค้ด' class="cf_barcode" href="#"><i class="fas fa-tags fa-lg" aria-hidden="true"></i></a>
                                </td>
                        ...
                        INNER JOIN color ON pp.col_id = color.col_id)
                        INNER JOIN rm_code ON pp.rm_id = rm_code.rm_id)
                        INNER JOIN roll_type ON pp.roll_id = roll_type.roll_id WHERE(pack_date='$date_pack' AND from_dp='$dp_frm' AND CONCAT(depart,' $',LPAD(pack_bill,2,'0'),' #',LPAD(box_id,3,'0'),' ',IF(status = 0,'+n','+y'),' ',prod_name,' ',col_code,' ',rm_code,' ',roll_type,' ',prod_type) like '%$search%') ORDER BY (box_id*1) DESC LIMIT $start,$limit";
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td align="center">
                                        <?php if ($arr['status'] == 0) { ?>
                                            <a href="#" title='Edit' class="cf_edit text-info" data-id="<?php echo ($autoBoxid) ?>"><i class="fas fa-edit" aria-hidden="true"></i></a>&nbsp;&nbsp;

                                            <a href="#" title='Delete' class="cf_delete text-danger" data-id="<?php echo ($autoBoxid) ?>" data-show="<?php echo (" กล่อง [ #" . $arr['box_id'] . "] ของวันที่ " . datedmY($date_pack)) ?>"><i class="fas fa-trash-alt" aria-hidden="true"></i></a>
                                        <?php } else { ?>
					...
					<?php $_link = "rm_pack_admin?Search=$search&packdate=$date_pack&pack_bill=$p_bill&from_dp=$dp_frm&page="; ?>
            ...
            <?php if ($submit == "Add") { ?>
                <div class="col-md-12">
                    <form name="fmAdd" id="frmAdd" method="post" action="rm_pack_admin?submit=OK&show=OK&packdate=<?php echo ($date_pack) ?>&from_dp=<?php echo ($dp_frm) ?>&Search=<?php echo ($search) ?>">
                        <div class="form-group row">
                            <div class="input-group mb-2 col-md-3">
                                <div class="input-group-prepend">
                                    <label class="input-group-text " for="boxid" style="background-color: aquamarine">เลขกล่อง</label>
                                </div>
                            ...
                            <div class="input-group mb-2 col-md-3">
                                <div class="input-group-prepend">
                                    <label class="input-group-text " for="pack_bill" style="background-color: aquamarine">เลขบิล</label>
                                </div>
                                <input name="pack_bill" type="number" id="pack_bill" class="form-control input-no-spinner" value="<?php echo $p_bill ?>" required tabIndex="2">
                            </div>

                            <div class="input-group mb-2 col-md-6">
                                <div class="input-group-prepend">
                                    <label class="input-group-text" for="to_dp">ผู้รับ</< /label>
                                </div>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="input-group mb-2 col-md-6">
                                <div class="input-group-prepend">
                                    <label class="input-group-text" for="colid">สี</label>
                                </div>
                        ...
                        <div class="form-group row">
                            <div class="input-group mb-2 col-md-6">
                                <div class="input-group-prepend">
                                    <label class="input-group-text" for="rm">วัตถุดิบ</< /label>
                                </div>
                            ...
                            <div class="input-group mb-2 col-md-6">
                                <div class="input-group-prepend">
                                    <label class="input-group-text" for="roll">ชนิดหลอด</label>
                                </div>
                        ...
                        <div class="form-group row">
                            <div class="input-group mb-2 col-md-6">
                                <div class="input-group-prepend">
                                    <label class="input-group-text " for="numbox" style="background-color: aquamarine">จำนวนหลอด</label>
                                </div>
                                <input name="numbox" type="number" id="numbox" value="" class="form-control" placeholder="0" step="1" tabIndex="6">
                            </div>
                            <div class="input-group mb-2 col-md-6">
                                <div class="input-group-prepend">
                                    <label class="input-group-text " for="kg" style="background-color: aquamarine">น้ำหนัก(ก.ก.)</label>
                                </div>
                                <input name="kg" type="number" value="" id="kg" class="form-control" step="0.01" placeholder="0.00" tabIndex="7">
                            </div>
                            <div class="input-group mb-2 col-md-6">
                                <div class="input-group-prepend">
                                    <label class="input-group-text" for="in_dex" style="background-color: aquamarine">Index</label>
                                </div>
                        ...
                        <div class='form-group'>
                            <div class='col-md-12' align="center">
                                ...
                                &nbsp;&nbsp;
                                <button type='button' class='btn btn-danger' onClick="document.location.href='rm_pack_admin?show=OK&packdate=<?php echo ($date_pack) ?>&pack_bill=<?php echo ($p_bill) ?>&from_dp=<?php echo ($dp_frm) ?>&to_dp=<?php echo ($to_dp) ?>&Search=<?php echo ($search) ?>'">ยกเลิก</button>
                            </div>
                        </div>
                    </form>
                </div>
            ...
            <?php
            if ($submit == "Edit") {
                $sqlEdit = "SELECT * FROM pro_pack WHERE(pack_date = '$date_pack' AND from_dp='$dp_frm' AND box_id = '$Sel_Boxid')";
            ...
            ?>
                <div class="col-md-12">
                    <form name="fmEdit" id="frmEdit" method="post" action="rm_pack_admin?submit=OK&show=OK&selboxid=<?php echo ($Sel_Boxid) ?>&packdate=<?php echo ($date_pack) ?>&bo=<?php echo ($box_id_old) ?>&from_dp=<?php echo ($dp_frm) ?>&Search=<?php echo ($search) ?>">
                        <div class="form-group row">
                            <div class="input-group mb-2 col-md-3">
                                <div class="input-group-prepend">
                                    <label class="input-group-text " for="boxid" style="background-color: aquamarine">เลขกล่อง</< /label>
                            ...
                            <div class="input-group mb-2 col-md-3">
                                ...
                                <input name="pack_bill" type="number" id="pack_bill" class="form-control input-no-spinner" value="<?php echo ($rowEdit['pack_bill']) ?>" required tabIndex="2">
                            </div>
                            <div class="input-group mb-2 col-md-6">
                                <div class="input-group-prepend">
                                    <label class="input-group-text" for="to_dp">ผู้รับ</< /label>
                                ...
                                <select class="custom-select" id="to_dp" name="to_dp">
                                    ...
                                    while ($arr_2 = mysqli_fetch_array($rstTemp)) {
                                    ?>
                                        <option value="<?php echo ($arr_2['id_depart']) ?>" <?php if ($tdp == $arr_2['id_depart']) {
                                                                                                echo ('selected');
                                                                                            } ?>>
                                            <?php echo ($arr_2['depart']) ?>
                                        </option>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="input-group mb-2 col-md-6">
                                <div class="input-group-prepend">
                                    <label class="input-group-text " for="kg" style="background-color: aquamarine">น้ำหนัก(Kg.)</label>
                                </div>
                        ...
                        <div class='form-group'>
                            <div class='col-md-12' align="center">
                                ...
                                <button type='button' class='btn btn-danger' onClick="document.location.href='rm_pack_admin?show=OK&packdate=<?php echo ($date_pack) ?>&from_dp=<?php echo ($dp_frm) ?>&Search=<?php echo ($search) ?>'">ยกเลิก</button>
                            </div>
                        </div>
                    </form>
                </div>
    ...
    <script>
        ...
        $(document).on('click', '.cf_delete', function(e) {
            ...
            var frmdp = '---';
            var s1 = document.getElementsByName('from_dp');
            frmdp = s1.item(0).value;
            ...
            if (frmdp != "---") {
                        ...
                        callback: function(result) {
                            if (result) {
                                window.location.href = 'rm_pack?submit=DEL&show=OK&packdate=' + pdate +
                                    '&selboxid=' + id + '&from_dp=' + frmdp + '&Search=' + sch;
                            }
                        }
                    });
                })
                ...
                $(function() {
                    bootbox.alert("กรุณาเลือกผู้บรรจุวัตถุดิบ ก่อนลบข้อมูล !!",
                        function() {});
                })
            }
        ...
        $(document).on('click', '.cf_add', function(e) {
            ...
            if (frmdp != "---") {
                $(function() {
                    window.location.href = 'rm_pack_admin?submit=Add&packdate=' + pdate + '&show=&mbid=' + mbox + '&from_dp=' + frmdp + '&Search=' + sch;
                })

            } else {
                $(function() {
                    bootbox.alert("กรุณาเลือกผู้บรรจุวัตถุดิบ ก่อนเพิ่มข้อมูล !!",
                        function() {});
                })
            }
        ...
        $(document).on('click', '.cf_edit', function(e) {
            ...
            if (frmdp != "---") {
                $(function() {
                    window.location.href = 'rm_pack_admin?submit=Edit&show=&packdate=' + pdate + '&selboxid=' + id + '&from_dp=' + frmdp + '&Search=' + sch;
                })

            } else {
                $(function() {
                    bootbox.alert("กรุณาเลือกบรรจุวัตถุดิบ ก่อนแก้ไขข้อมูล !!",
                        function() {});
                })
            }
        ...
        $(document).on('click', '.cf_barcode', function(e) {
            ...
            if (frmdp == "---") {
                $(function() {
                    bootbox.alert("กรุณาเลือกผู้บรรจุวัตถุดิบ ก่อนเลือกพิมพ์ใบข้างกล่อง !!",
                        function() {});
                })
            } else {
                $(function() {
                    window.location.href = 'rm_barcode_sel_admin?submit=&show=&packdate=' + pdate + '&from_dp=' + frmdp + '&Search=' + sch;
