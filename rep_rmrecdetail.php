...
<table width="750" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    <td align="center" class="headTitle">รายงานการรับวัตถุดิบ (<?=$departAcc?>)</td>
  </tr>
  <tr>
    <td align="center" class="headTitle2">วันที่รับ : <?=$date_fm ?>&nbsp;&nbsp;ถึง&nbsp;&nbsp;<?=$date_to ?></td>
  </tr>
  <tr>
    <td align="center" class="headTitle2"><?="ผู้บรรจุ :".$_depart?>&nbsp;&nbsp;&nbsp;&nbsp;คำค้นหา : ( &nbsp;<?=$ss ?>&nbsp; )</td>
  </tr>
</table>
...
FROM (SELECT CONCAT(prod_name, ' ', col_code, ' ', rm_code, ' ', roll_type) AS prodname, box_id, (pr.val * prod_kg) AS valA, roll_n, prod_kg, pr.prod_id AS prID, pr.col_id AS colID, pr.rm_id AS rmID, pr.roll_id as rollID, pr.status AS sst
    ...
    WHERE((rec_date BETWEEN '$date_fm2' AND '$date_to2') AND from_dp ='$dp_id' AND to_dp ='$dp_Acc' AND pr.status > 0 AND CONCAT('$',pack_bill,' #',LPAD(box_id,3,'0'),' #s',shelf_name,' ',shelf_desc,' ',mov_type,' ', IF(pr.status = 1,'+รับ',IF(pr.status = 2,'+เก็บ','+จ่าย')),' p',DATE_FORMAT(pack_date, '%d/%m/%Y'),' #s',shelf_name,' ',prod_name,' ',col_code,' ',rm_code,' ',roll_type,' ',prod_type) LIKE '%$ss%')) AS TT
GROUP BY ProdName
...
if($resultG && $resultG->num_rows>0){  // คิวรี่ข้อมูล Group สำเร็จหรือไม่ และมีรายการข้อมูลหรือไม่
    while($rowG = $resultG->fetch_assoc()){ // วนลูปแสดงรายการ Group
          ...
          $sql = "SELECT pack_date, pack_bill, box_id, CONCAT(prod_name,' ',col_code,' ',rm_code,' ',roll_type) AS prodname, pr.prod_id, pr.col_id, pr.rm_id, pr.roll_id, roll_n, prod_kg, d1.depart AS frm_dp, prod_name, col_code, rm_code, roll_type, pr.status, val, shelf_name
          FROM((((((pro_pack AS pr
                        INNER JOIN depart AS d1 ON from_dp = id_depart)
                    INNER JOIN (product INNER JOIN prod_type ON product.prod_type_id = prod_type.prod_type_id) ON pr.prod_id = product.prod_id)
                  INNER JOIN color ON pr.col_id = color.col_id)
            ...
            INNER JOIN roll_type ON pr.roll_id = roll_type.roll_id)
          INNER JOIN (shelf INNER JOIN moving_type ON sh_type_id = mov_id) ON pr.shelf_id = shelf.shelf_id)
          WHERE((rec_date BETWEEN '$date_fm2' AND '$date_to2') AND from_dp ='$dp_id' AND to_dp ='$dp_Acc' AND pr.status > 0 AND CONCAT(prod_name,' ',col_code,' ',rm_code,' ',roll_type) = '$strGroup' AND CONCAT('$',pack_bill,' #',LPAD(box_id,3,'0'),' #s',shelf_name,' ',shelf_desc,' ',mov_type,' ', IF(pr.status = 1,'+รับ',IF(pr.status = 2,'+เก็บ','+จ่าย')),' p',DATE_FORMAT(pack_date, '%d/%m/%Y'),' #s',shelf_name,' ',prod_name,' ',col_code,' ',rm_code,' ',roll_type,' ',prod_type) LIKE '%$ss%')
      ...
      <tr>
        <td align="left">
          <table width="750" border="0" align="left" cellpadding="0" cellspacing="0" style="border-collapse:collapse;border-top:2px solid #000;">
            <tr>
              <td width="70" height="25px" class="headerTabel" align="center" valign="middle">บรรจุ</td>
...
<!--====================== Footdata Report ========================= -->
    ...
    <table width="750" border="0" align="center" cellpadding="0" cellspacing="0">
        ...
        <tr>
          ...
          <td colspan="2" align="center" class="FooterR">ผู้รับเข้า..............................</td>
