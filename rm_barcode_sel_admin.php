	...
	<div class="container">
		<div class="row">
			<div class="col-md-12" style='margin-bottom: 8px; font-size:18px;'>
				<span class="d-block p-2 bg-secondary text-white" align="center"><?php echo $_SESSION['s_depart'] ?> - เลือกรายพิมพ์ใบข้างกล่อง </span>
			</div>
			...
			if ($submit == "" or $show == "OK") { ?>
				<!-- =========================== Show Form ================================== -->
				<div class="col-md-12">
					<form name="fmSearch" id="fmSearch" method="post" action="rm_barcode_sel_admin.php?show=OK&packdate=<?php echo ($date_pack) ?>&from_dp=<?php echo ($dp_frm) ?>" role='Search'>
...
WHERE(pack_date='$date_pack' AND from_dp='$dp_frm' AND pr.status = 0 AND CONCAT(RIGHT(YEAR(pack_date),2),LPAD(MONTH(pack_date),2,'0'),LPAD(DAY(pack_date),2,'0'),LPAD(from_dp,2,'0'),LPAD(box_id,3,'0'),' p',DATE_FORMAT(pack_date, '%d/%m/%Y'),' #',LPAD(box_id,3,'0'),' ',d1.depart,' ','$',pack_bill,' ',prod_name,' ',col_code,' ',rm_code,' ',roll_type,' ',prod_type) like '%$search%')";
					...
					<form name="frmrep" id="frmrep" method="post" action='' target="_blank">
						<div class="form-group row">
							...
							<div class="col-md-3" align="center" style="padding-bottom: 10px">
								...
								<a title='กลับหน้าบรรจุวัตถุดิบ' name="cfgoback1" href="rm_pack_admin?submit=&show=OK&packdate=<?php echo ($date_pack) ?>&from_dp=<?php echo ($dp_frm) ?>" class='btn btn-warning'>&nbsp;<i class="fa fa-undo" aria-hidden="true"></i>&nbsp;</a>
							...
							<div class="col-md-12">
								<table class="table table-striped table-responsive">
									...
									<tbody>
										<?php
										$tran_sql = "SELECT pack_date, pack_bill, from_dp ,box_id, roll_n, prod_kg, d1.depart AS frm_dp, prod_name, col_code, rm_code, roll_type, CONCAT(prod_name,' ', col_code,' ', rm_code) AS nameProd, pr.status, CONCAT(RIGHT(YEAR(pack_date),2),LPAD(MONTH(pack_date),2,'0'),LPAD(DAY(pack_date),2,'0'),LPAD(from_dp,2,'0') ,LPAD(box_id,3,'0')) AS b_name
FROM((((pro_pack AS pr INNER JOIN depart AS d1 ON to_dp = id_depart)
            INNER JOIN (product INNER JOIN prod_type ON product.prod_type_id = prod_type.prod_type_id) ON pr.prod_id = product.prod_id)
        INNER JOIN color ON pr.col_id = color.col_id)
    INNER JOIN rm_code ON pr.rm_id = rm_code.rm_id)
INNER JOIN roll_type ON pr.roll_id = roll_type.roll_id
WHERE(pack_date='$date_pack' AND from_dp='$dp_frm' AND pr.status = 0 AND CONCAT(RIGHT(YEAR(pack_date),2),LPAD(MONTH(pack_date),2,'0'),LPAD(DAY(pack_date),2,'0'),LPAD(from_dp,2,'0'),LPAD(box_id,3,'0'),' p',DATE_FORMAT(pack_date, '%d/%m/%Y'),' #',LPAD(box_id,3,'0'),' ',d1.depart,' ','$',pack_bill,' ',prod_name,' ',col_code,' ',rm_code,' ',roll_type,' ',prod_type) like '%$search%') ORDER BY (box_id *1) LIMIT $start,$limit";
								...
								<?php $_link = "rm_barcode_sel_admin?Search=$search&packdate=$date_pack&from_dp=$dp_frm&page="; ?>
