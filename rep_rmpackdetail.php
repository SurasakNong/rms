...
<head>
  <meta charset="utf-8">
  <title>รายงานการบรรจุวัตถุดิบ</title>
...
<table width="750" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    <td align="center" class="headTitle">รายงานการบรรจุวัตถุดิบ (<?=$_depart?>)</td>
  </tr>
  <tr>
    <td align="center" class="headTitle2">วันที่ : <?=$date_fm ?>&nbsp;&nbsp;ถึง&nbsp;&nbsp;<?=$date_to ?></td>
  </tr>
  <tr>
...
$tpi = 46; //จำนวนบรรทัดข้อมูลเมื่อไม่มีหัวหรือท้ายรายงาน
$total_page_item = $tpi; // จำนวนรายการที่แสดงสูงสุดในแต่ละหน้า
$total_page_item_all = 0; // จำนวนรายการจริงทั้งหมด
...
FROM (SELECT d1.depart AS frm_dp, CONCAT(prod_name, ' ', col_code, ' ', rm_code, ' ', roll_type) AS prodname, box_id, (pr.val * prod_kg) AS valA, roll_n, prod_kg, pr.prod_id AS prID, pr.col_id AS colID, pr.rm_id AS rmID, pr.roll_id as rollID, pr.status AS sst
    FROM ( ( ( ( ((pro_pack AS pr
                      INNER JOIN depart AS d1 ON from_dp = d1.id_depart )
    ...
    WHERE((pack_date BETWEEN '$date_fm2' AND '$date_to2') AND from_dp ='$dp_id' AND CONCAT(d2.depart,' $',LPAD(pack_bill,2,'0'),' #',LPAD(box_id,3,'0'),' ', IF(pr.status = 0,'+n','+y'),' ',IF(pr.status = 1,'+รับ',IF(pr.status = 2,'+เก็บ','+จ่าย')),' ',prod_name,' ',col_code,' ',rm_code,' ',roll_type,' ',prod_type) LIKE '%$ss%')) AS TT
GROUP BY ProdName
...
if($resultG && $resultG->num_rows>0){  // คิวรี่ข้อมูล Group สำเร็จหรือไม่ และมีรายการข้อมูลหรือไม่
    while($rowG = $resultG->fetch_assoc()){ // วนลูปแสดงรายการ Group
          ...
          $sql = "
          SELECT pack_date, pack_bill, box_id, from_dp, CONCAT(prod_name,' ',col_code,' ',rm_code,' ',roll_type) AS prodname, pr.prod_id, pr.col_id, pr.rm_id, pr.roll_id, roll_n, prod_kg, d1.depart AS frm_dp, prod_name, col_code, rm_code, roll_type, pr.status, val
          FROM((((((pro_pack AS pr
                        INNER JOIN depart AS d1 ON from_dp = d1.id_depart)
                        INNER JOIN depart AS d2 ON to_dp = d2.id_depart)
                    INNER JOIN (product INNER JOIN prod_type ON product.prod_type_id = prod_type.prod_type_id) ON pr.prod_id = product.prod_id)
              ...
              INNER JOIN rm_code ON pr.rm_id = rm_code.rm_id)
            INNER JOIN roll_type ON pr.roll_id = roll_type.roll_id)
          WHERE((pack_date BETWEEN '$date_fm2' AND '$date_to2') AND from_dp ='$dp_id' AND CONCAT(prod_name,' ',col_code,' ',rm_code,' ',roll_type) = '$strGroup' AND CONCAT(d2.depart,' $',LPAD(pack_bill,2,'0'),' #',LPAD(box_id,3,'0'),' ', IF(pr.status = 0,'+n','+y'),' ',IF(pr.status = 1,'+รับ',IF(pr.status = 2,'+เก็บ','+จ่าย')),' ',prod_name,' ',col_code,' ',rm_code,' ',roll_type,' ',prod_type) like '%$ss%')
    ...
    $total_page_data = ceil(($total_page_item_all+$row_head_rep)/$total_page_item); // หาจำนวนหน้าจากรายการทั้งหมด
  }
      ...
      <tr>
        <td align="left">
          <table width="750" border="0" align="left" cellpadding="0" cellspacing="0" style="border-collapse:collapse;border-top:2px solid #000;">
            <tr>
              <td width="80" height="25px" class="headerTabel" align="center" valign="middle">บรรจุ</td>
              ...
              <td width="50" class="headerTabel_thin" align="center" valign="middle">สถานะ</td>
              <td width="60" class="headerTabel_thin" align="center" valign="middle">ind.@</td>
              <td width="100" class="headerTabel_thin" align="center" valign="middle">index</td>
        ...
        // ส่วนของ repeat content
            for($v=1;$v<=$total_page_item;$v++){
              ...
              $_h1 = isset($arr_data_set['h1'][$item_i])?$arr_data_set['h1'][$item_i]:"";
              $_header = isset($arr_data_set['Header'][$item_i])?$arr_data_set['Header'][$item_i]:"";
              ...
              if ($_prodname !=''){
                if($_G1 == 'g1') { ?>
                  <tr>
                    <td colspan="5" width="550" height="20" align="right" class="lbg"><?=$_prodname.' ( '.number_format($_box_n).' กล่อง )'?>&nbsp;&nbsp; </td>
              ...
              <?php  }else { ?>
                  <tr>
                    <td align="center" class="left_bottom"><?=$_pack_date?></td>
                    <td align="center" class="left_bottom_thin"><?=$_packbill.'-'.$_box_id?></td>
...
<!--====================== Footdata Report ========================= -->
    ...
    <table width="750" border="0" align="center" cellpadding="0" cellspacing="0">
        ...
        <tr>
          <td colspan="2" align="center" class="FooterR">ผู้บรรจุ................................</td>
