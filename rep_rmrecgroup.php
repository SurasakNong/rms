...
<head>
  <meta charset="utf-8">
  <title>รายงานสรุปการรับวัตถุดิบ</title>
...
<div align="center">
  <table width="700" border="0" align="center" cellpadding="0" cellspacing="0">
    <tr>
      <td align="center" class="headTitle">รายงานสรุปการรับวัตถุดิบ (<?=$departAcc?>)</td>
    </tr>
    <tr>
...
FROM (SELECT CONCAT(prod_name, ' ', col_code, ' ', rm_code, ' ', roll_type) AS prodname, box_id, (pr.val * prod_kg) AS valA, roll_n, prod_kg, pr.prod_id AS prID, pr.col_id AS colID, pr.rm_id AS rmID, pr.roll_id as rollID, pr.status AS sst
    ...
    WHERE((rec_date BETWEEN '$date_fm2' AND '$date_to2') AND from_dp ='$dp_id' AND to_dp ='$dp_Acc' AND pr.status > 0 AND CONCAT('$',pack_bill,' #',LPAD(box_id,3,'0'),' ', IF(pr.status = 1,'+รับ',IF(pr.status = 2,'+เก็บ','+จ่าย')),' p',DATE_FORMAT(pack_date, '%d/%m/%Y'),' #s',shelf_name,' ',shelf_desc,' ',mov_type,' ',prod_name,' ',col_code,' ',rm_code,' ',roll_type,' ',prod_type) LIKE '%$ss%')) AS TT
GROUP BY ProdName
