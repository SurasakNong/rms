...
<head>
  <meta charset="utf-8">
  <title>รายงานสรุปการบรรจุวัตถุดิบ</title>
...
<div align="center">
  <table width="750" border="0" align="center" cellpadding="0" cellspacing="0">
    <tr>
      <td align="center" class="headTitle">รายงานสรุปการบรรจุวัตถุดิบ (<?=$_depart?>)</td>
    </tr>
    <tr>
...
FROM (SELECT d1.depart AS frm_dp, CONCAT(prod_name, ' ', col_code, ' ', rm_code, ' ', roll_type) AS prodname, box_id, (pr.val * prod_kg) AS valA, roll_n, prod_kg, pr.prod_id AS prID, pr.col_id AS colID, pr.rm_id AS rmID, pr.roll_id as rollID, pr.status AS sst
    ...
    WHERE((pack_date BETWEEN '$date_fm2' AND '$date_to2') AND from_dp ='$dp_id' AND CONCAT(d2.depart,' $',pack_bill,' #',LPAD(box_id,3,'0'),' ', IF(pr.status = 0,'+n','+y'),' ',IF(pr.status = 1,'+รับ',IF(pr.status = 2,'+เก็บ','+จ่าย')),' ',prod_name,' ',col_code,' ',rm_code,' ',roll_type,' ',prod_type) LIKE '%$ss%')) AS TT
GROUP BY ProdName
...
<!--=================== Data Report ============================ -->
      <tr>
        <td align="center">
          <table width="700" border="0" align="center" cellpadding="0" cellspacing="0" style="border-collapse:collapse;border-top:2px solid #000;">
            <tr>
              <td width="70" height="25px" class="headerTabel" align="center" valign="middle">ลำดับ</td>
              <td width="230" class="headerTabel_thin" align="center" valign="middle">รายการ</td>
            ...
            for($v=1;$v<=$total_page_item;$v++){
             ...
             if ($_prodname !=''){ ?>
                  <tr>
                    ...
                    <td align="left" width="300" height="20" class="left_bottom_thin">&nbsp;&nbsp;&nbsp;<?=$_prodname?></td>
