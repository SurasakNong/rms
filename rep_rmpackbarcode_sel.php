...
<body>
  ...
  for ($i = 1; $i <= $Num_Box; $i++) {
    ...
    if ($strStatus == "1") {
      ...
      $sql = "SELECT box_id, pack_date, pack_bill, box_id,d1.depart as nameDepart, from_dp,LPAD(from_dp,2,'0') AS fromdp, CONCAT(RIGHT(YEAR(pack_date),2),LPAD(MONTH(pack_date),2,'0'),LPAD(DAY(pack_date),2,'0'),LPAD(from_dp,2,'0'),LPAD(box_id,3,'0')) AS b_name,LPAD(box_id,3,'0') AS bbb, CONCAT(prod_name,' ',col_code,' ',rm_code,' ',roll_type) AS prodname, roll_n, prod_kg, d1.depart AS frm_dp, prod_name, col_code, rm_code, roll_type, pr.status, val
          FROM(((((pro_pack AS pr
                        INNER JOIN depart AS d1 ON from_dp = id_depart)
                    INNER JOIN product ON pr.prod_id = product.prod_id)
          ...
          WHERE(CONCAT(RIGHT(YEAR(pack_date),2),LPAD(MONTH(pack_date),2,'0'),LPAD(DAY(pack_date),2,'0'),LPAD(from_dp,2,'0') ,LPAD(box_id,3,'0')) = '$B_name')
